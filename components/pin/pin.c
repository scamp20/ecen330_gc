...
int32_t pin_output(pin_num_t pin, bool enable)
{
	...
	if (pin < 32) {
		...
		enable ? REG_SET_BIT(GPIO_ENABLE1_W1TS_REG, pin-REG_BITS) : REG_SET_BIT(GPIO_ENABLE1_W1TC_REG, pin-REG_BITS);
	}
	return 0;
}
...
int32_t pin_set_level(pin_num_t pin, int32_t level)
{
	...
	if (pin < 32) {
		...
		level ? (REG_SET_BIT(GPIO_OUT1_W1TS_REG, pin-REG_BITS)) : (REG_SET_BIT(GPIO_OUT1_W1TC_REG, pin-REG_BITS));
	}
	return 0;
}
...
uint64_t pin_get_in_reg(void)
{
	...
	return (uint64_t)REG(GPIO_IN_REG) + ((uint64_t)REG(GPIO_IN1_REG) << REG_BITS);
}
...
uint64_t pin_get_out_reg(void)
{
	...
	return (uint64_t)REG(GPIO_OUT_REG) + ((uint64_t)REG(GPIO_OUT1_REG) << REG_BITS);
}
